...
| --- | --- | --- | --- |
| SYNC-01 | Importer commit `ab08f58` (lovable-sync) inn i Lovable: 14 filer + 3 prosjektdokumenter | **Delvis** | 14 filer hentet fra GitHub og skrevet inn; vitest 468/468 i 42 filer, tsc og build kjørt i denne runden. Gjenstår: reviewbar Lovable-commit bekreftet av bruker |
| SAVE-01 | «Lagre tur»-knapp i turinnstillingene: fast ved skjemabunn, safe-area, ≥44 px, «Lagrer…», blokkert dobbeltklikk, feil/retry, «Lagret» først etter serverbekreftelse + oppdatert delt turtilstand | **Åpen** | Kode importert (AdminTrips.tsx, TripContext.applySavedTrip) og dekket av `trip-save-runtime.test.tsx`. Produksjonsfeilen er **ikke** lukket: mangler verifisering i faktisk app på iPhone og på en annen autorisert enhet |
