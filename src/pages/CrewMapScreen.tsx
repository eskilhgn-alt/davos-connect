/**
 * CrewMapScreen ("Magnus?") — full-screen map with scrollable user list below
 * Features: place search, distance indicators, tap-to-navigate, re-center button
 */
...
export const CrewMapScreen: React.FC = () => {
  ...
  // Re-center on own location when first available
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&viewbox=9.6,46.7,10.1,46.9&bounded=1&limit=5`
        );
  ...
  return (
    ...
    >
      ...
      {/* Full scrollable content: map on top, list below */}
      <div
      ...
      >
        ...
        <div className="relative" style={{ height: "55vh", minHeight: 300 }}>
          ...
          <button
            ...
            className="absolute top-3 left-3 z-[500] bg-card/95 backdrop-blur-sm border border-border rounded-lg p-2.5 shadow-sm active:scale-95 transition-transform"
            aria-label="Søk sted"
          >
          ...
          {myLoc && (
            <button
              onClick={recenter}
              className="absolute top-3 right-14 z-[500] bg-card/95 backdrop-blur-sm border border-border rounded-lg p-2.5 shadow-sm active:scale-95 transition-transform"
          ...
          {searchOpen && (
            <div className="absolute inset-0 z-[600] bg-background/95 backdrop-blur-sm flex flex-col p-4">
              ...
              {searching && (
                <p className="text-xs text-muted-foreground px-1">Søker...</p>
              )}
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-border">
                {searchResults.map((r, i) => (
                  <button
                    ...
                    className="w-full text-left px-2 py-3 hover:bg-muted/50 active:bg-muted transition-colors"
                  >
            ...
            <div className="px-4 pb-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 rounded-lg border border-border bg-muted/30 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                />
              </div>
            </div>
          )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">
                      {loc.display_name}
                      {isMe && (
                        <span className="text-xs text-muted-foreground ml-1">(deg)</span>
                      )}
                    </p>
