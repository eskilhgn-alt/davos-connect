 ...
 * Edit profile, send password reset, manage roles, notes, toggle admin
 */
...
export const AdminUserDetail: React.FC<Props> = ({ user: u, currentUserId, onRefresh, onLogAction }) => {
  ...
  const toggleActive = async () => {
    ...
    try {
      ...
      onLogAction(currentUserId, u.is_active ? "user_deactivated" : "user_activated", u.id);
      onRefresh();
  ...
  const sendPasswordReset = async () => {
    if (newPassword.length < 6) { errorToast("Passord må være minst 6 tegn"); return; }
    setLoading("reset");
    try {
  ...
  const toggleBan = async () => {
    ...
    try {
      ...
      onLogAction(currentUserId, u.is_banned ? "user_unbanned" : "user_banned", u.id);
      onRefresh();
    } catch (e: any) {
      ...
      <DavosCardContent className="p-4">
        <button onClick={() => setExpanded(!expanded)} className="w-full flex items-start justify-between text-left gap-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <DavosAvatar src={u.avatar_url || undefined} fallback={u.nickname || u.full_name || u.email} size="md" />
          ...
          <div className="mt-4 space-y-4 border-t border-border pt-4">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              ...
              <div>
                ...
                <p className="font-medium flex items-center gap-1">
                  {u.email_verified ? (<><CheckCircle size={12} className="text-green-500" /> Ja</>) : (<><XCircle size={12} className="text-destructive" /> Nei</>)}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Rolle</p>
                <p className="font-medium flex items-center gap-1"><Shield size={12} /> {u.role === "admin" ? "Admin" : "Bruker"}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Registrert</p>
                <p className="font-medium flex items-center gap-1"><Calendar size={12} /> {new Date(u.created_at).toLocaleDateString("nb-NO")}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Status</p>
                <p className="font-medium">{u.is_banned ? "🚫 Utestengt" : u.is_active ? "✅ Aktiv" : "⏸️ Inaktiv"}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="col-span-2 flex gap-2">
                <DavosInput type="password" placeholder="Nytt passord" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="flex-1 h-8 text-xs" />
              ...
              <DavosButton variant={u.is_active ? "outline" : "primary"} size="sm" onClick={toggleActive} disabled={loading === "toggle" || u.id === currentUserId}>
                {loading === "toggle" ? <Loader2 size={14} className="animate-spin mr-1" /> : u.is_active ? <UserX size={14} className="mr-1 text-destructive" /> : <UserCheck size={14} className="mr-1" />}
                {u.is_active ? "Deaktiver" : "Aktiver"}
              </DavosButton>
              <DavosButton variant={u.is_banned ? "primary" : "outline"} size="sm" onClick={toggleBan} disabled={loading === "ban" || u.id === currentUserId}>
                {loading === "ban" ? <Loader2 size={14} className="animate-spin mr-1" /> : u.is_banned ? <UserCheck size={14} className="mr-1" /> : <ShieldOff size={14} className="mr-1 text-destructive" />}
              ...
              <DavosButton variant="outline" size="sm" onClick={toggleAdminRole} disabled={loading === "role" || u.id === currentUserId}
              ...
              {!u.email_verified && (
                <DavosButton variant="outline" size="sm"
                  onClick={async () => {
                    setLoading("verify");
                    try {
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <DavosButton variant="outline" size="sm" disabled={u.id === currentUserId || loading === "delete"}
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      <strong>{u.nickname || u.full_name || u.email}</strong> vil bli fullstendig fjernet. Denne handlingen kan ikke angres.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
