...
export const CurrencyCalculator: React.FC<CurrencyCalculatorProps> = ({
  ...
  const converted =
    direction === "foreign-to-nok"
      ? (numAmount * rate).toFixed(2)
      : (numAmount / rate).toFixed(2);

  const fromCurrency = direction === "foreign-to-nok" ? foreign : "NOK";
  const toCurrency = direction === "foreign-to-nok" ? "NOK" : foreign;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div
        className="relative w-full max-w-sm mx-4 mb-8 sm:mb-0 bg-background rounded-2xl shadow-xl border border-border overflow-hidden animate-in slide-in-from-bottom-4 duration-200"
      ...
      >
        ...
        <div className="px-5 pb-6 space-y-4">
          ...
          <div className="flex justify-center">
            <button
              onClick={() =>
                setDirection((d) => (d === "foreign-to-nok" ? "nok-to-foreign" : "foreign-to-nok"))
              }
          ...
          <p className="text-[10px] text-muted-foreground/50 text-center">
            1 {foreign} = {rate.toFixed(4)} NOK · Den europeiske sentralbanken
            {rateDate && (
              <>
                <br />
                Kurs fra:{" "}
                {new Date(rateDate + "T00:00:00").toLocaleDateString("nb-NO", {
                  weekday: "short",
                  day: "2-digit",
                  month: "2-digit",
                  year: "numeric",
            ...
            {rateFetchedAt && (
              <>
                <br />
                Hentet:{" "}
                {rateFetchedAt.toLocaleString("nb-NO", {
                  weekday: "short",
                  day: "2-digit",
                  month: "2-digit",
                  hour: "2-digit",
